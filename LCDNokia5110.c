...
static uint8_t masterRxData = 0U;
static uint8_t masterTxData = 0U;
static dspi_transfer_t masterXfer;
static dspi_master_config_t  masterConfig;

void LCDNokia_init(void) {
	/*Configuracion para inicializar el SPI Maestro */
	...
	/*Funciones para inicializar el clk con el que vamos a trabajar   */
	uint32_t srcClock_Hz = CLOCK_GetFreq(DSPI_MASTER_CLK_SRC);
	/*  Funciones para incializar el SPI Master */
	DSPI_MasterGetDefaultConfig(&masterConfig);
	DSPI_MasterInit(DSPI_MASTER_BASEADDR, &masterConfig, srcClock_Hz);
	
	/* Configuramos los pines de reset y comand como pines de salida */
	gpio_pin_config_t reset_pin_config = { kGPIO_DigitalOutput, 0,};
	gpio_pin_config_t data_command_pin_config = { kGPIO_DigitalOutput, 0,};

	GPIO_PinInit(GPIOD, RESET_PIN, &reset_pin_config);
	GPIO_PinInit(GPIOD, DATA_OR_CMD_PIN, &data_command_pin_config);


	GPIO_ClearPinsOutput(GPIOD, 1u << RESET_PIN);
	LCD_delay();
	GPIO_SetPinsOutput(GPIOD, 1u << RESET_PIN);
...
void LCDNokia_writeByte(uint8_t DataOrCmd, uint8_t data)
{
	if(DataOrCmd)
	{
		GPIO_SetPinsOutput(GPIOD, 1u << DATA_OR_CMD_PIN);
	}
	else
	{
		GPIO_ClearPinsOutput(GPIOD, 1u << DATA_OR_CMD_PIN);
	}
	

	/* Realizamos la configuracion pra establecer que informacion se va a transmitir, el Chipselect y el tamaño de transferencia*/
...
void LCDNokia_sendChar(uint8_t character) {
  ...
  //Blank vertical line padding can be added for extra space between words
